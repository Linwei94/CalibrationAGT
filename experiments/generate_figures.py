...
This script predates the current manuscript framing in `paper/main.tex` and
still uses older labels such as `ECE-Hard` and `ECE-Soft`.
Keep it only for historical exploration; the paper itself is aligned to
`ECE_voted` and `ECE_true`, and the figures used in the manuscript are
generated by the dedicated scripts under `paper/` plus
`experiments/run_toy_example.py`.
...
----------------
  ...
  fig5_calibration_gap.pdf — legacy summary of ECE-Soft − ECE-Hard per method

Usage
-----
    # After running the CIFAR-10H experiment:
...
def fig_summary(results: dict, out_path: str):
    ...
    # Annotate the discrepancy for TS
    if "TS" in names:
        i = names.index("TS")
        gap = ece_s[i] - ece_h[i]
    ...
    ax.set_title("ECE-Hard vs. ECE-Soft on CIFAR-10H\n"
                 "TS appears calibrated on hard labels but diverges under soft-label evaluation",
                 fontweight="bold")
...
def fig_stratified(results: dict, out_path: str):
    ...
    ax.set_title("ECE-Soft Stratified by Ambiguity Level (CIFAR-10H)\n"
                 "The discrepancy is largest for high-ambiguity inputs",
                 fontweight="bold")
    ax.legend()
    ax.grid(True, axis="y", alpha=0.25)
